...
import { resetMonthlyUsage, generateMonthlyInvoice, getCompanySubscription, } from "../services/billing/billingService.js";
/**
 * MONTHLY BILLING JOB
 *
...
// ====================================================================
export async function executeMonthlyBillingJob() {
    ...
    try {
      ...
      `);
        ...
        console.log(`[BILLING JOB] Processing ${companies.length} companies...`);
        for (const { id: companyId } of companies) {
            try {
                ...
                console.log(`[BILLING JOB] Resetting usage for company: ${companyId}`);
                await resetMonthlyUsage(companyId);
                console.log(`[BILLING JOB] ✓ Completed for company: ${companyId}`);
            }
            catch (error) {
                console.error(`[BILLING JOB] Error processing company ${companyId}:`, error);
                errors.push({
                    companyId,
                    error: error.message,
                });
        ...
        console.log(`[BILLING JOB] Summary: ${companies.length} companies, ${invoicesGenerated} invoices`);
        return {
            success: true,
...
// ====================================================================
async function checkUsageAnomalies(companyId, invoice) {
    try {
        const subscription = await getCompanySubscription(companyId);
        if (!subscription)
            return;
        const promptExceeded = invoice.promptTokensUsed >
            subscription.plan.monthlyPromptTokens * 1.2; // 20% overage threshold
        const completionExceeded = invoice.completionTokensUsed >
            subscription.plan.monthlyCompletionTokens * 1.2;
        if (promptExceeded || completionExceeded) {
            console.warn(`[BILLING JOB] Usage anomaly detected for company: ${companyId}`);
            await pool.query(`
        ...
        console.error(`[BILLING JOB] Error checking anomalies for ${companyId}:`, error);
    }
}
// ====================================================================
// NOTIFY COMPANY ABOUT INVOICE
// ====================================================================
async function notifyCompanyAboutInvoice(companyId, invoice) {
    try {
      ...
      `, [companyId]);
        ...
        const emailContent = formatInvoiceEmail(name, invoice, plan_name);
        ...
        //   html: emailContent,
        // });
        console.log(`[BILLING JOB] Invoice notification queued for: ${email}`);
    }
    catch (error) {
        console.error(`[BILLING JOB] Error notifying company ${companyId}:`, error);
    }
}
// ====================================================================
// FORMAT INVOICE EMAIL
// ====================================================================
function formatInvoiceEmail(companyName, invoice, planName) {
...
// ====================================================================
export async function triggerMonthlyBillingJobManually(companyId) {
    try {
        if (companyId) {
            console.log(`[BILLING JOB] Manually triggering for company: ${companyId}`);
            const invoice = await generateMonthlyInvoice(companyId);
            await notifyCompanyAboutInvoice(companyId, invoice);
            await resetMonthlyUsage(companyId);
            console.log(`[BILLING JOB] ✓ Completed for company: ${companyId}`);
...
export async function executeCleanupJob() {
    try {
      ...
      `);
        console.log(`[CLEANUP JOB] Deleted ${auditDeleteResult.rowCount} old audit entries`);
