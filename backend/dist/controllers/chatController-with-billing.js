...
import { extractGeminiUsageMetadata, } from "../middleware/usageTracking.middleware.js";
                    ...
                    try {
                        ...
                        res.write(`data: ${JSON.stringify({
                            _metadata: {
                                ...
                                completionTokens,
                                totalTokens,
                                costCents: Math.ceil((promptTokens / 1000) * 1 +
                                    (completionTokens / 1000) * 4),
                            },
                        })}\n\n`);
                        console.log(`[Token Usage] company=${companyId} | tokens=${totalTokens} | ` +
                            `prompt=${promptTokens} | completion=${completionTokens}`);
                    }
                    catch (trackingError) {
                        console.error("Error recording token usage:", trackingError.message);
 ...
 */
export const chatNonStreaming = async (req, res) => {
    try {
        const { question, sessionId: requestedSessionId, session_id, } = req.body;
        const companyId = req.user?.companyId;
        const employeeId = req.employee?.id;
