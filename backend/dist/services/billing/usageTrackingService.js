...
// ====================================================================
export function calculateTokenCost(promptTokens, completionTokens, promptRatePer1k, completionRatePer1k) {
    const promptCostCents = Math.ceil((promptTokens / 1000) * promptRatePer1k);
    const completionCostCents = Math.ceil((completionTokens / 1000) * completionRatePer1k);
...
export async function checkUsageLimits(params) {
    ...
    const promptPercentUsed = Math.round((projectedPromptUsage / monthlyPromptLimit) * 100);
    const completionPercentUsed = Math.round((projectedCompletionUsage / monthlyCompletionLimit) * 100);
    const currentUsagePercent = Math.max(promptPercentUsed, completionPercentUsed);
    const promptTokensRemaining = Math.max(0, monthlyPromptLimit - projectedPromptUsage);
    const completionTokensRemaining = Math.max(0, monthlyCompletionLimit - projectedCompletionUsage);
    const tokensRemaining = Math.min(promptTokensRemaining, completionTokensRemaining);
...
export async function enforceUsageLimits(params) {
    ...
    if (!limitStatus.isWithinLimit) {
        throw new LimitExceededError(params.companyId, limitStatus.currentUsagePercent, "monthly");
    }
}
// ====================================================================
// GET CURRENT MONTH USAGE
// ====================================================================
export async function getCurrentMonthUsage(companyId) {
...
// GET TODAY'S USAGE
// ====================================================================
export async function getTodayUsage(companyId, employeeId) {
    const today = new Date().toISOString().split("T")[0];
    let query = `
...
// CREATE USAGE ALERT
// ====================================================================
export async function createUsageAlert(companyId, alertType, currentUsage, limitValue, period) {
...
// ====================================================================
export async function markAlertAsNotified(alertId) {
    await pool.query(`
...
// ====================================================================
export async function getUsageLogs(companyId, limit = 100, offset = 0) {
    const result = await pool.query(`
