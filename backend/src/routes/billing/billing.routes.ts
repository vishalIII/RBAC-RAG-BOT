...
import {
  ...
  getTopEmployeesByUsage,
  getUsageByDepartment,
  getUsageTrend7Days,
  getPlanLimitStatus,
  getUsageBreakdown,
  getCostProjection,
} from "../../services/billing/analyticsService.js";
import {
 ...
 */
router.get("/plans", authenticate, async (_req: Request, res: Response) => {
  try {
 ...
 */
router.post(
  "/admin/trigger-billing",
  authenticate,
  authorize("platform_admin"),
  async (req: Request, res: Response) => {
    try {
      const { executeMonthlyBillingJob } = await import(
        "../../jobs/monthlyBillingJob.js"
      );
      ...
      if (companyId) {
        ...
        const { generateMonthlyInvoice, resetMonthlyUsage } = await import(
