...
export async function getCompanyUsageStats(
  ...
  const currentCompletionTokens = parseInt(
    currentMonth?.completion_tokens || 0,
  );
  ...
  return {
    ...
    currentMonth: {
      ...
      percentOfLimit: Math.min(
        100,
        Math.round((totalTokens / monthlyLimit) * 100),
      ),
    },
    previousMonth: {
...
// ====================================================================
export async function getTopEmployeesByUsage(
  companyId: string,
  limit: number = 10,
): Promise<EmployeeUsageStats[]> {
...
// ====================================================================
export async function getUsageByDepartment(companyId: string): Promise<
  Array<{
    departmentId: string;
...
// ====================================================================
export async function getUsageTrend7Days(companyId: string): Promise<
  Array<{
    date: string;
    promptTokens: number;
    completionTokens: number;
...
export async function getPlanLimitStatus(
  ...
  const promptPercentUsed = Math.round(
    (row.prompt_tokens_used / row.monthly_prompt_tokens) * 100,
  );
  const completionPercentUsed = Math.round(
    (row.completion_tokens_used / row.monthly_completion_tokens) * 100,
  );
...
// ====================================================================
export async function getCostProjection(companyId: string): Promise<{
  basePlanCost: number;
  ...
  const projectedCompletion =
    completionUsed + dailyCompletionAvg * daysRemaining;

  // Calculate costs
  const basePlanCost = subscription.plan.monthlyPriceCents;

  const promptOverage = Math.max(
    0,
    projectedPrompt - subscription.plan.monthlyPromptTokens,
  );
  const completionOverage = Math.max(
    0,
    projectedCompletion - subscription.plan.monthlyCompletionTokens,
  );

  const overageCost = Math.ceil(
    (promptOverage / 1000) * 1 + (completionOverage / 1000) * 4,
  );
...
// ====================================================================
async function getCompanySubscription(companyId: string): Promise<
  | {
      plan: {
