...
export async function generateMonthlyInvoice(companyId: string): Promise<BillingInvoice> {
  ...
  const promptOverage = Math.max(
    0,
    promptTokensUsed - subscription.plan.monthlyPromptTokens
  );
  const completionOverage = Math.max(
    0,
    completionTokensUsed - subscription.plan.monthlyCompletionTokens
  );
  ...
  if (promptOverage > 0 || completionOverage > 0) {
    ...
    const { totalCents } = calculateTokenCost(
      promptOverage,
      completionOverage,
